...
// This make the widget matches to the desiredRect
	...
	virtual void		minSize(const glm::dim2& sz);
...
// This does not change the size of widget directly
	virtual void		resize(const glm::vec2& p,const glm::dim2& sz);
	virtual void		resize(const glm::rect& r) { resize(r.tl(),r.wh() ); }
	virtual void		size(const glm::dim2& sz);
	virtual void		position(const glm::vec2& p);
	virtual void		x(float v){ position(glm::vec2(v,y())); }
	virtual void		y(float v){ position(glm::vec2(x(),v)); }
	virtual void		w(float v){ size(glm::dim2(v,h())); }
	virtual void		h(float v){ size(glm::dim2(w(),v)); }
...
// Those functions are to be used in the enclousers or subclasses
	virtual glm::rect	bound() const { return glm::rect(position(),size()); }
	virtual glm::rect	contentsRect() const { return _contentsRect; }
	virtual glm::dim2	size() const { return changed()?_setSize:_bound.wh(); }
	virtual glm::vec2	position() const { return _bound.tl(); }
	...
	virtual glm::dim2		minSize() const { return _minSize; }
	...
	virtual glm::rect 		paddedRect() const { return glm::rect(leftPadding(),topPadding(),w()-horzPadding(),h()-vertPadding()); }
	...
	virtual glm::idim2		abelImageSize() const { return _labelImageSize; }
	virtual bool		labelImageFit() const { return _labelImageFit; }
	virtual glm::dim2		measureTextLabel(NVGcontext* vg) const;
...
// Those function is used by the enclouser and the JGL
	...
	virtual glm::vec2	abs_pos() const;
	virtual glm::rect	abs_rect() const {return glm::rect( abs_pos(), size() ); }

	virtual ~Widget();
	
	virtual glm::irect	prevViewport() const { return _prevViewport; }
	virtual glm::irect	prevScissor() const { return _prevScrissor; }
	...
	Widget(const glm::vec2& p, const glm::dim2& sz, const string& label="" );
...
protected:
	...
	virtual glm::idim2	getWindowSize() const;
...
// Geometric configuration
	glm::rect	_bound;
	glm::rect	_contentsRect;

	glm::dim2	_setSize;
	glm::dim2	_minSize			= glm::dim2(10,10);
	float		_topPadding			= 0;
...
// Image label state
	...
	glm::idim2	_labelImageSize		= glm::idim2(0,0);
