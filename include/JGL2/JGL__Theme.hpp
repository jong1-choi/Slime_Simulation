...
struct Theme {
	...
	virtual glm::dim2	_dim_check_action_size					()const;
	virtual glm::dim2	_dim_options_action_min_size			()const;
	...
	virtual float		_size_options_button_width				(const glm::dim2& r)const;
	virtual float		_size_options_arrow_hwidth				(const glm::dim2& r)const;
	virtual float		_size_options_arrow_y_offset			(const glm::dim2& r)const;
	virtual float		_size_options_arrow_height				(const glm::dim2& r)const;
	...
	virtual glm::dim2	_measure_tooltip_text(NVGcontext* vg, const string& s)const;
	virtual void		_draw_tooltip(NVGcontext* vg, const glm::rect& p, float dx, const string& str, float alpha )  const;
	virtual glm::vec2	_vec_tooltip_offset						()const;
	
	
	virtual void		_draw_textinputbase_box(NVGcontext* vg, const glm::rect& r, int state);
	virtual void		_draw_textinputbase_selection(NVGcontext* vg, const glm::rect& r, const glm::rect& sr, float x0, float x1, int state);
	virtual void		_draw_textinputbase_cursor(NVGcontext* vg, const glm::rect& r, const glm::rect& sr, float x, int state);
	virtual void		_draw_textinputbase_text(NVGcontext* vg, const glm::rect& r, const glm::rect& sr, const string& str, int align, int state);
	virtual glm::dim2	_measure_textinputbase_text(NVGcontext* vg, const string& str);
	
	virtual float		_pos_slider_tick						(const glm::rect& r) const;
	virtual float		_pos_slider_number						(int type, const glm::rect& r) const;
	...
	virtual float		_pos_f_slider_y							(int type,const glm::rect&r)const;
	virtual void		_draw_slider_cursor						(NVGcontext* vg,int type,const glm::rect& r,float x,int s)const;
	virtual void		_draw_slider_ticks_minor				(NVGcontext* vg,int type,const glm::rect& r,int s,float offset,float step)const;
	virtual void		_draw_slider_ticks_major				(NVGcontext* vg,int type,const glm::rect& r,int s,float offset,float step)const;
	virtual void		_draw_slider_ticks_fine					(NVGcontext* vg,int type,const glm::rect& r,int s,float offset,float step)const;
	virtual void		_draw_slider_numbers					(NVGcontext* vg,int type,const glm::rect& r,int s,float offset,float step, float valueOffset,float valueStep,double base)const;
	virtual void		_draw_slider_bar						(NVGcontext* vg,int type,const glm::rect& r,int s) const;
};
...
inline float _size_options_button_width(const glm::dim2& r) {
	return Theme::getCurrentTheme()->_size_options_button_width(r);
}
inline float _size_options_arrow_y_offset(const glm::dim2& r) {
	return Theme::getCurrentTheme()->_size_options_arrow_y_offset(r);
}
inline float _size_options_arrow_height(const glm::dim2& r) {
	return Theme::getCurrentTheme()->_size_options_arrow_height(r);
}
inline float _size_options_arrow_hwidth(const glm::dim2& r) {
...
inline glm::dim2 _dim_options_action_min_size() {
	return Theme::getCurrentTheme()->_dim_options_action_min_size();
}
inline glm::dim2 _dim_check_action_size() {
...
inline glm::dim2	_measure_tooltip_text(NVGcontext* vg,  const string& s) {
	return Theme::getCurrentTheme()->_measure_tooltip_text(vg, s);
}

inline glm::vec2	_vec_tooltip_offset() {
	return Theme::getCurrentTheme()->_vec_tooltip_offset();
}


inline void _draw_textinputbase_box(NVGcontext* vg, const glm::rect& r, int s) {
	Theme::getCurrentTheme()->_draw_textinputbase_box(vg,r,s);
}
inline void _draw_textinputbase_selection(NVGcontext* vg, const glm::rect& r, const glm::rect& sr, float x0, float x1, int s) {
	Theme::getCurrentTheme()->_draw_textinputbase_selection(vg,r,sr,x0,x1,s);
}
inline void _draw_textinputbase_cursor(NVGcontext* vg, const glm::rect& r, const glm::rect& sr, float x, int s) {
	Theme::getCurrentTheme()->_draw_textinputbase_cursor(vg,r,sr,x,s);
}
inline void _draw_textinputbase_text(NVGcontext* vg, const glm::rect& r, const glm::rect& sr, const string& str, int align, int s) {
	Theme::getCurrentTheme()->_draw_textinputbase_text(vg,r,sr,str,align,s);
}
inline glm::dim2 _measure_textinputbase_text(NVGcontext* vg, const string& str) {
...
inline float _pos_slider_tick (const glm::rect& r) {
...
inline float	_pos_f_slider_y(int type,const glm::rect&r) {
	return Theme::getCurrentTheme()->_pos_f_slider_y(type,r);
}
inline float	_pos_slider_number(int type,const glm::rect&r) {
	return Theme::getCurrentTheme()->_pos_slider_number(type,r);
}



inline void		_draw_slider_cursor(NVGcontext* vg,int type,const glm::rect& r,float x,int s) {
	return Theme::getCurrentTheme()->_draw_slider_cursor(vg,type,r,x,s);
}
inline void		_draw_slider_ticks_major(NVGcontext* vg,int type,const glm::rect& r,int s,float offset,float step) {
	return Theme::getCurrentTheme()->_draw_slider_ticks_major(vg,type,r,s,offset,step);
}
inline void		_draw_slider_ticks_minor(NVGcontext* vg,int type,const glm::rect& r,int s,float offset,float step) {
	return Theme::getCurrentTheme()->_draw_slider_ticks_minor(vg,type,r,s,offset,step);
}
inline void		_draw_slider_ticks_fine(NVGcontext* vg,int type,const glm::rect& r,int s,float offset,float step) {
	return Theme::getCurrentTheme()->_draw_slider_ticks_fine(vg,type,r,s,offset,step);
}
inline void		_draw_slider_numbers(NVGcontext* vg,int type,const glm::rect& r,int s,float offset,float step, float valueOffset,float valueStep,double base) {
	return Theme::getCurrentTheme()->_draw_slider_numbers(vg,type,r,s,offset,step,valueOffset,valueStep,base);
}
inline void _draw_slider_bar(NVGcontext* vg,int type,const glm::rect& r,int s)  {
