...
static void glnvg__getUniforms(GLNVGshader* shader)
{
	...
	// Joony
	shader->loc[GLNVG_LOC_OUTCOLORMAT] = glGetUniformLocation(shader->prog, "outColorMat");
	shader->loc[GLNVG_LOC_OUTGAMMA] = glGetUniformLocation(shader->prog, "outGamma");
	// End Joony
...
static int glnvg__renderCreate(void* uptr)
{
	...
	static const char* fillFragShader =
		...
		"#endif\n"
		// Joony
		"	uniform mat3 outColorMat;\n"
		"	uniform vec3 outGamma;\n"
		// End Joony
		...
		"}\n"
		"vec3 tonemap(vec3 u, vec3 g ) {\n"
		"	return vec3(tonemap(u.r,g.r),tonemap(u.g,g.g),tonemap(u.b,g.b));\n"
		"}\n"
		"vec3 invTonemap(vec3 u, vec3 g ) {\n"
		"	return vec3(invTonemap(u.r,g.r),invTonemap(u.g,g.g),invTonemap(u.b,g.b));\n"
		"}\n"
		"void main(void) {\n"
		"   vec4 result;\n"
		...
		"	}\n"
		// Joony
		"	result.rgb = tonemap(outColorMat*invTonemap(result.rgb,vec3(2.4)),outGamma);\n"
		// End Joony
		"#ifdef NANOVG_GL3\n"
		"	outColor = result;\n"
		"#else\n"
							   ...
							   float width, float fringe, float strokeThr)
{
	...
	// Joony
	memcpy(frag->gamma, gamma, sizeof(frag->gamma));
	memcpy(frag->colorMat, colorMat, sizeof(frag->colorMat));
	// End Joony
...
static void glnvg__setUniforms(GLNVGcontext* gl, int uniformOffset, int image)
{
	...
	// Joony TODO:
	GLNVGfragUniforms* frag = nvg__fragUniformPtr(gl, uniformOffset);
	glUniformMatrix3fv(gl->shader.loc[GLNVG_LOC_OUTCOLORMAT], 1, 0, frag->colorMat);
	glUniform3fv(gl->shader.loc[GLNVG_LOC_OUTGAMMA], 1, frag->gamma);
	// End Joony
...
static void glnvg__renderFill(void* uptr, NVGpaint* paint, NVGcompositeOperationState compositeOperation, NVGscissor* scissor,
							  float gamma[3], float colorMat[3],
							  float fringe,
								   ...
								   const NVGvertex* verts, int nverts, float fringe)
{
	...
	if (call->uniformOffset == -1) goto error;
	frag = nvg__fragUniformPtr(gl, call->uniformOffset);
	glnvg__convertPaint(gl, frag, paint, scissor,
						gamma, colorMat,
						1.0f, fringe, -1.0f);
