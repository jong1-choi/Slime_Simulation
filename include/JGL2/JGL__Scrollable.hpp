...
struct _Scrollable {
	virtual void		scrollRange( const glm::dim2& screen, const glm::rect& content, int align );
	virtual void		scrollOffset(const glm::vec2& d);
	virtual glm::vec2	scrollOffset() const;
...
protected:
	...
	glm::vec2	_scrollOffset = glm::vec2(0,0);
	bool		_mouseOnHorzScroller = false;
	bool		_mouseOnVertScroller = false;
	glm::vec2	_scrollClicked;
	glm::rect	_scrollWidgetBound;
	glm::rect	_scrollContentsBound;
	glm::vec2	_scrollRange;
	glm::vec2	_scrollLengthes;
	int			_align;
