...
//
	virtual void		drawAction(NVGcontext* vg, const glm::rect& r, int alignment) override;
	virtual bool		handle(int event) override;


	virtual glm::dim2	minActionSize(NVGcontext*vg) const override;
...
template<typename T> inline  void Slider<T>::drawAction(NVGcontext* vg, const glm::rect& r, int alignment) {
	if( gridDamaged() ) {
		updateGridStep();
		_gridDamaged = false;
	}
	...
	_cursorPt = glm::vec2( valueToX( value() ), _pos_f_slider_y(type, r) );
	...
	if( _valueTipAlpha>0 ) {
		...
		if(  0 > _cursorPt.y-sz.h-10+abs_pos().y ) {
			...
			rr = glm::rect(_cursorPt.x-sz.w/2-3, _cursorPt.y-sz.h-10, sz.w+6, sz.h+2);
			nvgShadowRect(vg, rr, 10, glm::vec2(0,0), _radius_tooltip_box()*1.5f);
			nvgPathBalloon(vg,rr,glm::vec2(_cursorPt.x-rr.x,rr.h+10),_radius_tooltip_box(),glm::dim2(6,5));
...
template<typename T> inline glm::dim2 Slider<T>::minActionSize(NVGcontext*vg) const {
	return glm::dim2(30,25);
}
...
template<typename T> inline T Slider<T>::xToValue( float x ) const {
	...
	if( !logScale() )
		return T(glm::mix( minValue(), maxValue(), xx ));
	else
		return T(pow(10,glm::mix( log10(minValue()), log10(maxValue()), xx ) ));
}
