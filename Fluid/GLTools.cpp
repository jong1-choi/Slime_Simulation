...
void drawObject(const std::vector<glm::vec3>& v, const std::vector<glm::vec3>& n, const std::vector<glm::uvec3>& f, glm::vec3 p, glm::vec4 color) {
	glm::mat4 modelMat = glm::translate(p);
	...
	if (!mesh.va) {
		...
		glDeleteBuffers(1, &mesh.nBuf);
		glDeleteBuffers(1, &mesh.eBuf);
		mesh.va = 0;
		mesh.vBuf = 0;
		mesh.nBuf = 0;
		mesh.eBuf = 0;
