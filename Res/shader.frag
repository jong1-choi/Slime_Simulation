...
uniform vec2 lightRadius = vec2(100);
...
void main(void) {
	...
	float lightInt = max(0,dot(N,L))*visibility*PI/dot(dl,dl)*100000;
	//out_Color = vec4(0);
	out_Color = vec4( pow(c*lightInt+am,vec3(1/2.2))  + .8f * color.rgb, color.a);
}
